...
impl ParseManager {
    ...
    pub async fn parse_now(&self, path: &Path) -> Result<Option<Tree>> {
        ...
        let Some((_id, language)) = LanguageManager::language_for_path(&abs) else {
            return Ok(None);
        };
        ...
        let new_tree =
            task::spawn_blocking(move || parser.parse(text.as_bytes(), old_tree.as_ref())).await?;
